...
namespace OnlineShop.Controllers
{
    public class ProfileController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> UpdateProfile(ProfileModel model, IFormFile avatarFile)
        {


            var check = await _userDAO.GetUser(model.Email);
        ...
        public async Task<IActionResult> UpdatePassword(ChangePasswordModel model)
        {
            ...
            if (!await authenticateUser.checkResetPass(user.Password, model.oldPassword))
            {
            ...
            Md5 md5 = new Md5();
            user.Password = await md5.MD5Create(model.newPassword);
        ...
        public async Task<IActionResult> DeleteAccount()
        {
            try
            {
                User currentUser = await GetCurrentLoggedInUser();
                if (currentUser != null)
                {
                    if (currentUser.Role == 2)
                    {
                        await _userDAO.Remove(currentUser.UserId);
                    }
        ...
        public async Task<IActionResult> SendPass(string email)
        {
            User user = await GetCurrentLoggedInUser();
            var subject = "http://localhost:5057/ForgotPassword/ResetPassword/" + user.UserId;
            bool check = await mailer.Send(email, "Yêu cầu khôi phục mật khẩu hiện tại", "Mật khẩu hiện tại của bạn là: Không cóa đâu. Mơ đi :))" + "Vào đây thay đổi nè: " + subject);
            if (check)
            {
