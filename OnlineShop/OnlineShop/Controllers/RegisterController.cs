...
namespace OnlineShop.Controllers
{
    public class RegisterController : Controller
    {
        ...
        public async Task<IActionResult> Index(RegisterModel model)
        {
            ...
            {
                if (await userDAO.GetUser(model.Email) != null)
                {
            ...
            {
                Md5 md5 = new Md5();
                string hashedPassword = await md5.MD5Create(model.Password);
                ...
                bool _check = await mailer.Send(model.Email, "Yêu cầu xác thực tài khoản", "Mã xác thực là: " + code);
                if (_check)
                {
