...
namespace OnlineShop.Controllers
{
    public class ForgotPasswordController : Controller
    {
        ...
        {
            var check = await userDAO.GetUser(model.Email);
            if (check == null)
            {
            ...
            bool _check = await mailer.Send(check.Email, "Yêu cầu thay đổi mật khẩu mới", "Hãy truy cập đường dẫn sau để thay đổi mật khẩu mới: " + subject);
            if (_check == true)
            {
        ...
        {
            var user = await userDAO.GetUserById(int.Parse(model.userID));
